...
class IndexingService:
    def __init__(self):
        ...
        self.config_mapping = {
            # 'text/plain': 'settings_txt.yaml', 
            # 'text/csv': 'settings_csv.yaml',
            # 'application/json': 'settings_json.yaml',
            # 'application/xml': 'settings_xml.yaml',
            # 'text/markdown': 'settings_md.yaml',
            # 'application/msword': 'settings_doc.yaml',
            # 'application/vnd.openxmlformats-officedocument.wordprocessingml.document': 'settings_docx.yaml',
            # 'application/vnd.ms-excel': 'settings_xls.yaml',
            # 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet': 'settings_xlsx.yaml',
            # 'application/vnd.ms-powerpoint': 'settings_ppt.yaml',
            # 'application/vnd.openxmlformats-officedocument.presentationml.presentation': 'settings_pptx.yaml',
            # 'application/pdf': 'settings_pdf.yaml',
            # 'image/jpeg': 'settings_image.yaml',
            # 'image/png': 'settings_image.yaml',
            # 'image/gif': 'settings_image.yaml',
            # 'image/bmp': 'settings_image.yaml',
            # 'image/tiff': 'settings_image.yaml',
            # 'application/zip': 'settings_zip.yaml',
            # 'application/x-tar': 'settings_tar.yaml',
            # 'application/gzip': 'settings_gz.yaml',
            # 'application/vnd.rar': 'settings_rar.yaml',
        }
        # 默认配置文件
    ...
    #YUZON 去 config_mapping 里找 file_type 对应的配置文件,如果找不到就用 self.default_config
    ...
    # 为每个用户建立专属的独立沙盒（input 和 output 文件夹）。这保证了张三的图谱不会混进李四的数据里
    ...
    # --------------------- 下面来两个函数是正式将文档进行索引，形成知识图谱---------------------
    # 微软的 GraphRAG 工具并不直接接收网络流。它需要你把所有的源文件老老实实地放在一个特定的 input 目录下。
    # 所以这个方法就是把 main.py 刚刚存好的文件，再复制一份到 GraphRAG 专属的 input/user_xxx/ 目录中
