...
# node.py
# │
# ├─ GraphRAGQueryInputState   # 输入状态
# ├─ GraphRAGQueryOutputState  # 输出状态
# ├─ GraphRAGAPI               # GraphRAG查询封装
# │    ├─ initialize()
# │    └─ query_graphrag()     #执行 GraphRAG 查询。#local 社区级搜索，entities、 relationships、 text_units
                                                   #global社区总结级搜索、速度更快
                                                   #drift 上下文漂移搜索
                                                   #basic 最简单查询，只使用：text_units
# │
# └─ create_graphrag_query_node()
#       └─ graphrag_query()   # LangGraph节点，返回：graphrag_query()
...
class GraphRAGAPI:
    def __init__(self, project_dir: str = None, 
        ...
        self.query_type = query_type or settings.GRAPHRAG_QUERY_TYPE  # YUZON  GraphRAG查询类型：local、global、drift、basic
    ...
    async def initialize(self):
        ...
        # 创建FilePipelineStorage对象，GraphRAG文件存储接口。
    ...
    async def query_graphrag(self, query: str) -> Dict[str, Any]:
        ...
        def on_context(context):                #YUZON 获取查询上下文。
            nonlocal context_data
            context_data = context
        
        local_callbacks = NoopQueryCallbacks()  #YUZON GraphRAG默认回调。
        local_callbacks.on_context = on_context
        callbacks.append(local_callbacks)
        
        try:
            # 根据查询类型执行不同的查询
            if self.query_type.lower() == "local":        #YUZON .lower()用于将字符串中所有大写字母转换为小写
            ...
            ## YUZON {
            #  "response": "OpenAI was founded by Sam Altman...",
            #  "context": {
            #      "entities": [...],
            #      "relationships": [...],
            #      "sources": [...]
