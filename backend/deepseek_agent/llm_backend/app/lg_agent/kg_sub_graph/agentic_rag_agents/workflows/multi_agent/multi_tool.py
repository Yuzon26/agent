...
def create_multi_tool_workflow(
    ...
    main_graph_builder.add_node("customer_tools", customer_tools) # GraphRAG Query 节点
    ...
    # TODO 这里应该注册一个 error_tool_selection ，因为tool_selection节点中有对于这个error_tool_selection的跳转逻辑
    ...
    main_graph_builder.add_conditional_edges(
        "guardrails",
        guardrails_conditional_edge,
        # 这个条件边没有指定“目的地列表”。这意味着这个函数内部会决定去向。如果这个函数返回 "planner"，就去往 planner
    )
    main_graph_builder.add_conditional_edges(
        "planner",
        map_reduce_planner_to_tool_selection,  # type: ignore[arg-type, unused-ignore]
        ["tool_selection"], # 指定了“目的地列表”
    )
