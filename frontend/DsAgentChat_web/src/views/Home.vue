    ...
    <div class="sidebar" :class="{ 'sidebar-collapsed': isSidebarCollapsed }">
      <div class="sidebar-header">
        <div class="logo-wrapper">
          <span class="logo-text"><span class="logo-highlight">MilitaryAgent</span></span>
      ...
      <div class="chat-content">
        ...
        <div v-if="!messages.length && !uploadedFile" class="initial-state">
          <div class="welcome-message">
            <h2>我是面向军事领域的Agent, 很高兴见到你!</h2>
            <p>我可以基于专业知识库来解决你的问题，请把你的任务交给我吧~</p>
          </div>
...
const sendMessage = async () => {
  ...
  if (!reader) throw new Error('No reader available')
    ...
    // 知识库问答
      ...
      const reader = await ApiService.rag([userMessage], conversationStore.currentConversationId!)
      if (!reader) throw new Error('No reader available')
      await handleChatStream(reader)
    } else {
  ...
  for (const file of input.files) {
    ...
    try {
      ...
      if (result.status === 'success') {
        //YUZON 不用micro graphrag 不用index
        // 保存 index_id  
...
const sendPopupMessage = async () => {
  ...
  try {
    ...
    formData.append('user_id', userId) // 直接传字符串或数字都可以
    
    // 如果有 conversation_id，则追加进去
    if (conversationStore.currentConversationId) {
      formData.append('conversation_id', conversationStore.currentConversationId.toString())
    }

    // 调用LangGraph接口
    const response = await fetch('http://localhost:8000/api/langgraph/query', {
      method: 'POST',
      // ⚠️ 注意：这里绝对不要写 headers: {'Content-Type': 'multipart/form-data'}
      // 浏览器发现 body 是 FormData 时，会自动帮你设置正确的头部并加上 boundary！
      body: formData
    })
...
.logo-highlight {
  background: linear-gradient(90deg, #d7d7e2 0%, #d8d8e2 100%);
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  font-weight: 600;
}
